...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    public function __construct($config = 'rest')
    {
        ...
        // filip: needed to set the correct offset to get the variables correctly according to our URI setup (with language code in the URI)
	...
	// Skip keys test for $config['auth_override_class_method']['class'['method'] = 'none'
        ...
        // filip: make sure that if api key was not found, it still tries alternative auth method
        //if ($this->auth_override !== true && !(config_item('rest_enable_keys') && $this->_allow === true)) {
        if ($this->auth_override !== true && !(config_item('rest_enable_keys') && $this->_allow !== false)) {
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        // filip: detect if key is empty, in that case don't get them out of here, just yet, try alternative auth method if available
        //if (config_item('rest_enable_keys') and $use_key and $this->_allow === false) {
        if (config_item('rest_enable_keys') and $use_key and $this->_allow === false and ! empty($this->rest->key)) {
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            ...
            // filip: set the current user in the application as well
            $this->users_model->set_current_user($row->user_id);
            $this->roles_model->set_current_user($row->user_id);
            $this->permissions_model->set_current_user($row->user_id);
            // filip: set constant to know API calls will not be done by a user within our system but from outside
            define('ACCESSED_VIA_API', TRUE);
    ...
    protected function _log_request($authorized = false)
    {
        ...
        if(isset($arguments['password'])){
            $arguments['password'] = "xxxxxxxxxx";
        }
        $status = $this->rest->db->insert(config_item('rest_logs_table'), array(
                    'uri' => $this->uri->uri_string(),
                    'method' => $this->request->method,
                    'params' => $arguments ? (config_item('rest_logs_json_params') ? json_encode($arguments) : serialize($arguments)) : '',
    ...
    function compress($source, $destination, $quality){
            ...
            $filename = tempnam(sys_get_temp_dir(), "vr");
    ...
    function s3_bucket_upload($temppath,$image_name,$scope,$site="web")    {
        ...
        try {
            ...
            if ($site == "web") {
                $result = $s3Client->putObject([
                    'Bucket' => $bucket,
                    'Key' => $scope.'/'.$image_name,
                    'SourceFile' => $file_Path,
                    'ACL' => 'public-read',
                ]);
                
                if($result['@metadata']['statusCode']=="200"){
                    $this->log_model->add('info', "File successfully uploaded to the S3 bucket. File Name(".$image_name."), Scope(".$scope.")");
                    $data['status'] = TRUE;
                    $data['image_name'] = $image_name;
                    $data['image_path'] = $result['ObjectURL'];
                }
                else{
                    $this->log_model->add('error', "File could not be uploaded to the S3 bucket. File Name(".$image_name."), Scope(".$scope.")");
                    
                    $failed_img = array();
                    $failed_img['img_full_path'] = $temppath;
                    $failed_img['img_name'] = $image_name;
                    $failed_img['scope'] = $scope;
                    $failed_img['operation'] = FAILED_TO_UPLOAD_FILE_ON_S3;
                    $failed_img['date_created'] = now();
                    $failed_img['user_created'] = $this->session->userdata('ds_user');
                    $this->log_model->save_failed_image_details($data);
                ...
                $upload = $s3Client->upload($bucket, $image_name, $temppath, 'public-read');
                $data['status'] = TRUE;
                $data['image_path'] = $upload->get('ObjectURL');
            }
        } 
        catch (Exception $e) {
            $this->log_model->add('error', "File could not be uploaded to the S3 bucket. File Name(".$image_name."), Scope(".$scope.")");
    ...
    public function download_file_from_s3($scope,$file_name,$file_old_name){        
        try {           
            ...
            $result = $client->getObject(array(
                ...
                'SaveAs'  => "./album_images/".$file_old_name
            ));            
            if(!empty($result)){
                $this->log_model->add('info', 'Album image successfully downloaded from S3. File name('.$file_name.'), scope('.$scope.')');
                return TRUE;
            }
            else{
                $this->log_model->add('error', 'Album image could not be download from S3. File name('.$file_name.'), scope('.$scope.')');
            }            
        } 
        catch (Exception $e) {
    ...
    public function unlink_file($file_name,$scope)
    {
        if($file_name && $scope){
            $file = "./public_html/upload/".$scope.'/'.$file_name;
            if(file_exists($file)){
                if (unlink($file)) {

                } else {
                    // write to system log
                    $this->log_model->add('error', 'File could not be delete from the server. File name('.$file_name.'), scope('.$scope.')');
                    
                    $failed_img = array();
                    $failed_img['img_full_path'] = $file;
                    $failed_img['img_name'] = $file_name;
                    $failed_img['scope'] = $scope;
                    $failed_img['operation'] = FAILED_TO_DELETE_FILE_FROM_SERVER;
                    $failed_img['date_created'] = now();
                    $failed_img['user_created'] = $this->session->userdata('ds_user');
                    $this->log_model->save_failed_image_details($data);
                }
    ...
    public function unlink_album_file($file_name)
    {
        if($file_name){
            $file = "./album_images/".$file_name;
            if(file_exists($file)){
                if (unlink($file)) {

                } else {
                    // write to system log
                    $this->log_model->add('error', 'Album image could not be delete from the server. File name('.$file_name.')');
