...
<?php } ?>
                        ...
                        <section class="vbox bg-gradient content-vbox">
                            ...
                            <section class="scrollable wrapper no-padding">
                                <div id="epaper_section" class="wrapper">
                                    <div class="col-md-12 no-padding">
                                        <a href="<?= site_url().DASHBOARD_DIR_NAME.'epaper/add'?>" class="btn btn-dark"><?= lang('add_new_epaper'); ?></a>
                                    </div>
                                    <div class="col-md-12 no-padding">
                                        <section class="panel panel-default no-padding" style="margin-top: 10px;">
                                            <header class="panel-heading">
                                                <i class="fa fa-epaperpaper-o"></i> &nbsp;<?= lang('epaper'); ?>
                                            </header>
                                            <div class="table-responsive">
                                                <table class="table table-striped m-b-none epaper_datatable">
                                                    <thead>
                                                        <tr>
                        ...
                        <script>                            
                            $(document).ready(function(e){
                                get_epaper_datatable();
                            });
                            function get_epaper_datatable(){
                                table = $('.epaper_datatable').DataTable({ 
                                    "processing": true, 
                                    "serverSide": true,
                                    "destroy" : true,
                                    "searching" : true,
                                    ...
                                    "ajax": {
                                       "url": "<?= site_url(); ?>api/epaper/list_epaper_datatable",
                                       "type": "POST"
                                    },
                                    "columnDefs": [
                                        { 
                                ...
                                $(".epaper_datatable_paginate").css("margin-top","0px");
                            ...
                            ?>
                                function delete_epaper(id){
                                    if(id){
                                        alertify.confirm("<?= lang('are_you_sure'); ?>", "<?= lang('do_you_want_to_delete_epaper'); ?>", function(){
                                            $.ajax({
                                                data: {id:id},
                                                type: "post",
                                                dataType: 'json',
                                                url: "<?= site_url() ?>api/epaper/delete_epaper_details",
                            ...
                            ?>
                                function changeHomeSliderStatus(id){
                                    ...
                                    $.ajax({
                                        ...
                                        type: "post",
                                        dataType: 'json',
                                        url: "<?= site_url() ?>api/epaper/change_epaper_home_slider_status",
                                        success: function (data) {
                                            if(data.status){
