...
class Live_News extends MY_Controller {
    ...
    public function index() {
        ...
        $this->load->view(DASHBOARD_DIR_NAME . 'page.live_news.php',$tmpl);
    }
    public function add($news_id = FALSE) {
        //check permissions
        $this->permissions_model->check_redirect('live_news','add');
        
        $tmpl = array();
        if($news_id){
            $tmpl['news_id'] = $news_id;
        }
        $tmpl['page_title'] = lang('add_new_live_news');
        $tmpl['live_news'] = $this->news_model->get_where($this->_news_table,array('deleted'=>'0','visibility'=>'1','parent_news_id'=>'0'));
        $tmpl['topics'] = $this->news_model->get_where($this->_topics_table,array('deleted'=>'0','visibility'=>'1'));
        
        $this->load->view(DASHBOARD_DIR_NAME . 'page.add_live_news.php',$tmpl);
    }
    public function view($url = FALSE)
    {   
        //check permissions
        $this->permissions_model->check_redirect('live_news','view');
        ...
        if($url){
            ...
            if(!empty($news_details)){
                ...
                $tmpl['topics'] = $this->news_model->get_where($this->_topics_table,array('deleted'=>'0','visibility'=>'1'));
                $tmpl['news_topics'] = $this->news_model->get_where($this->_topic_entities_trans_table,array('deleted'=>'0','entity'=>TOPICS_ENTITY_LIVE_NEWS,'entity_id'=>$news_details[0]['news_id']));
                $this->load->view(DASHBOARD_DIR_NAME.'page.view_live_news.php',$tmpl);
            }
            else{
