...
class Users extends REST_Controller
{
  ...
  //         $return = array('status' => TRUE, 'message' => lang('User deleted successfully'));
  //       } else {
  //         $return = array('status' => FALSE, 'message' => lang('msg_something_went_wrong'));
  ...
  // }
   ...
   public function delete_user_details_get()
  {
    ...
    try {
      ...
      if ($this->input->get('user_id')) {
        ...
        if ($res) {
          ...
          //$return = array('status' => FALSE, 'message' => lang('msg_something_went_wrong'));
          $return = array('status' => FALSE, 'message' => lang('User not valid'));
          $this->log_model->add('error', "User could not be deleted, user_id(" . $where['user_id'] . ")");
        }
        $this->response($return, 200);
      } else {
  ...
  public function save_user_subscription_plan_razorpay_post()
  {
    ...
    if($data['event']=='payment.failed')
    {
      ...
      $update_data = array("subscribed_plan_id" =>'0', "subscription_end_date" =>$yesterday, "subscription_payment_id" =>'');
      $res = $this->users_model->update_user_details($this->_users_table, $update_data, $where);
    } 
  ...
  public function contact_us_post()
  {
    ...
    try {
      ...
      // if ($this->input->post('user_id')) {
        ...
        $email_status = $this->email_notification_model->send_contact_us_details($name,$email,$contact_no,$message);
        //echo $email_status; die;
        if ($email_status) {

          $return = array('status' => TRUE, 'message' => lang('Your contact form successfully submitted'));
        } else {
          $return = array('status' => FALSE, 'message' => lang('msg_something_went_wrong'));
        }
        $this->response($return, 200);
