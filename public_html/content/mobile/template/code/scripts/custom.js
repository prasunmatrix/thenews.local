...
$(document).ready(function(){      
	...
	function init_template(){        
            ...
            $('[data-toggle-theme], [data-toggle-theme-switch]').off().on('click',function(){
                ...
                $('body').toggleClass('theme-light theme-dark');
                    ...
                    if($('body').hasClass('theme-light')){
                        ...
                        localStorage.setItem('PinkMirror-Theme','light-mode')
                    }
                    if($('body').hasClass('theme-dark')){
                        darkSwitch.prop('checked', true); 
                        localStorage.setItem('PinkMirror-Theme','dark-mode')
                    }
                setTimeout(function(){allowTransitions();},150);
                return false;
            })
            if (localStorage.getItem('PinkMirror-Theme') == "dark-mode") {darkSwitch.prop('checked', true); $('body').removeClass('theme-light').addClass('theme-dark');}
            if (localStorage.getItem('PinkMirror-Theme') == "light-mode") {darkSwitch.prop('checked', false); $('body').removeClass('theme-dark').addClass('theme-light');}
        ...
        localStorage.setItem('z_PinkMirror-Visited:' + window.location.pathname, 'Page Visited');
        $.fn.visited = function (className) {
            className = className || false;
            $(this).each(function (index, element) {
                if (element.host == window.location.host &&
                    localStorage.getItem('z_PinkMirror-Visited:' + element.pathname) ||
                    localStorage.getItem('z_PinkMirror-Visited:' + element.pathname + '/')) {
                    if (className) {
        ...
        $('.list-group a').on('click',function(){if($(this).attr('href') != '#'){localStorage.setItem('z_PinkMirror-Visited-Scroll-Position', $(window).scrollTop());}})
        if($('.check-visited').length){
            setTimeout(function(){
                var getScroll = localStorage.getItem('z_PinkMirror-Visited-Scroll-Position');
                $(window).scrollTop(getScroll)
                 localStorage.removeItem('z_PinkMirror-Visited-Scroll-Position');
            },200);
        }
        ...
        if(visitDetection.length){
            var lastVisitValue = localStorage.getItem('PinkMirror-Last-Visited');
            var d = new Date();
            ...
            if(!lastVisitValue){
                $('.never-visited').show();
                $('.before-visited').hide();
                localStorage.setItem('PinkMirror-Last-Visited', '<br>Your last visit was ' + strDate +' at '+ strTime)
            } else {
       ...
       $(".text-size-default").click(function() {$(".text-size-changer *").css("font-size", "");});
        ...
        setTimeout(function(){
            ...
            $('.single-slider').owlCarousel({loop:true, margin:20, nav:false, lazyLoad:true, items:1, autoplay: false, autoplayTimeout:4000});		
        ...
        function highlightLoader(){
            $('[data-highlight-style]').on('click',function(){
                ...
                localStorage.setItem('PinkMirror-Highlight', getHighlight);
                $('#highlight').remove();
                $('head').append( $('<link rel="stylesheet" id="highlight" type="text/css" />').attr('href', getHighlight) );
                return false;
            });
            var rememberHighlight = localStorage.getItem('PinkMirror-Highlight');
		...
		if($('.chart').length > 0){
			...
			var call_charts_to_page = function(){
                ...
                var doughnutChart2 = $('#doughnut-chart-2');
                var doughnutChart3 = $('#doughnut-chart-3');
                var doughnutChart4 = $('#doughnut-chart-4');
                ...
                if(doughnutChart.length){
                    var doughnutDemoChart = new Chart(doughnutChart, {
                        ...
                        data: {
                          labels: ["A", "C", "C", "VR"],
                          datasets: [{
                            backgroundColor: ["#000", "#5D9CEC","#FC6E51","#CCD1D9"],
                            borderColor:"rgba(255,255,255,0.5)",
                            data: [100,100,100,700]
                          }]
                        },
                        options: {
                            responsive: true, maintainAspectRatio:false,
                ...
                if(doughnutChart2.length){
                    var doughnutDemoChart = new Chart(doughnutChart2, {
                        type: 'doughnut',
                        data: {
                          labels: ["A", "C", "C", "VR"],
                          datasets: [{
                ...
                if(doughnutChart3.length){
                    var doughnutDemoChart = new Chart(doughnutChart3, {
                        type: 'doughnut',
                        data: {
                          labels: ["A", "C", "C", "VR"],
                          datasets: [{
                ...
                if(doughnutChart4.length){
                    var doughnutDemoChart = new Chart(doughnutChart4, {
                        type: 'doughnut',
                        data: {
                          labels: ["Rectangle Score", "Square Score", "Heart Score"],
                          datasets: [{
                            backgroundColor: ["#000", "#5D9CEC","#FC6E51"],
                            borderColor:"rgba(255,255,255,0.5)",
                            data: [3.8,3.3,2.9]
                          }]
                        },
			...
			loadJS('content/mobile/template/code/scripts/charts.js', call_charts_to_page, document.body);
		}        
        ...
        if(pwaEnabled === true){
            ...
            var dismissDate = localStorage.getItem('PinkMirror-PWA-Timeout-Value');
            if((day - dismissDate) > pwaRemind){
                localStorage.removeItem('PinkMirror-PWA-Prompt')
            } 
                
            //Dismiss Prompt Button
            $('.pwa-dismiss').on('click',function(){
                console.log('User Closed Add to Home / PWA Prompt')
                localStorage.setItem('PinkMirror-PWA-Prompt', 'install-rejected');
                $('body').find('#menu-install-pwa-android, #menu-install-pwa-ios, .menu-hider').removeClass('menu-active'); 
                localStorage.setItem('PinkMirror-PWA-Timeout-Value', day);
            });
            ...
            if (isMobile.Android()) {
                function showInstallPrompt(){
                    if($('#menu-install-pwa-android, .add-to-home').length){
                        if (localStorage.getItem('PinkMirror-PWA-Prompt') != "install-rejected") {
                            setTimeout(function(){
